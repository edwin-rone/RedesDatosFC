...
int main(int argc, char *argv[]) {
    // --- El servidor ahora toma su puerto como argumento ---
    if (argc != 2) {
    ...
    server_addr.sin_port = htons(server_port); // Usamos el puerto del argumento
    ...
    printf("[Servidor en puerto %d] Escuchando conexiones...\n", server_port);
    ...
    printf("[Servidor en puerto %d] Cliente conectado.\n", server_port);

    int bytes_received = recv(client_sock, buffer, BUFFER_SIZE - 1, 0);
    if (bytes_received <= 0) {
        fprintf(stderr, "[Servidor en puerto %d] Error al recibir datos o cliente desconectado.\n", server_port);
    } else {
        ...
        if (first_space != NULL) {
            ...
            if (second_space != NULL) {
                ...
                if (target_port == server_port) {
                    printf("[Servidor en puerto %d] Petición aceptada. Cifrando archivo...\n", server_port);
                    send(client_sock, "Archivo recibido y cifrado.", strlen("Archivo recibido y cifrado."), 0);

                } else {
                    printf("[Servidor en puerto %d] Petición rechazada (cliente pidió puerto %d).\n", server_port, target_port);
